<?php
	$origin = 'update mail set deleted=0b1 where mailid=?';
	$mailid = $_GET['mailid'];
	...
	if(!mysqli_stmt_bind_param($mysqli_stmt, 's', $mailid)) {
	...
	$origin = "insert into admin_log (adminid, mailid, operation, rdate) values (?, ?, 'MAKEDELETED', NULL)";
	if(!mysqli_stmt_prepare($mysqli_stmt, $origin)) {
		mysqli_rollback($mysqli);
		die('Statement Preparation Fail, Error: (' . mysqli_stmt_errno($mysqli_stmt) . ')' . mysqli_stmt_error($mysqli_stmt));
	}
